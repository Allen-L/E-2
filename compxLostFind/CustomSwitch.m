...
//        }
        ...
        if (self.status == CustomSwitchStatusOn) {
            if (status == CustomSwitchStatusOff) {
                [self moveButtonTranslation:0]; //原来是On，现在是Off，需要左移
            }
            
        }
        else{
            if (status == CustomSwitchStatusOn) {
                [self moveButtonTranslation:_minusTranslate];//原来是Off，现在是On，需要右移
            }
        }
    }
...
- (void)drawRect:(CGRect)rect
{
    if (!first) {
        ...
        [_customSwitch setFrame:CGRectMake(0, 0, _onImage.size.width, _onImage.size.height)];
    ...
    _leftRect = CGRectMake(-(_offImage.size.width - _offImage.size.height), 0, _onImage.size.width, _onImage.size.height);
    _middleRect = CGRectMake(0, 0, _onImage.size.width, _onImage.size.height);
    
//        if (self.status==CustomSwitchStatusOff) {
//            [_customSwitch addSubview:_offButton];
//            [_customSwitch addSubview:_onButton];
//        }
//        else if (self.status==CustomSwitchStatusOn){
//            [_customSwitch addSubview:_onButton];
//            [_customSwitch addSubview:_offButton];
//        }
...
//            }
    ...
    else
    {
        ...
        if (self.status==CustomSwitchStatusOff) {
            ...
            _middleRect = CGRectMake(0, 0, _offImage.size.width, _offImage.size.height);
            
            [_customSwitch addSubview:_offButton];
            [_customSwitch addSubview:_onButton];
        }
        else if (self.status==CustomSwitchStatusOn){
            _leftRect = CGRectMake(0, 0, _onImage.size.width, _onImage.size.height);
             _middleRect = CGRectMake((_offImage.size.width - _offImage.size.height), 0, _offImage.size.width, _offImage.size.height);
            [_customSwitch addSubview:_onButton];
            [_customSwitch addSubview:_offButton];
        ...
        if (_arrange == CustomSwitchArrangeONLeftOFFRight) {
            ...
            if (self.status == CustomSwitchStatusOn) {
                ...
                [self moveButtonTranslation:_minusTranslate];
                 [_customSwitch exchangeSubviewAtIndex:1 withSubviewAtIndex:0];
